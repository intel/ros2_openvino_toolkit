...
#include <string>
#include <memory>
#include "vino_core_lib/outputs/ros_topic_output.hpp"
#include "vino_core_lib/pipeline_params.hpp"
#include "vino_core_lib/pipeline.hpp"
...
: BaseOutput(output_name)
{
  ...
  pub_license_plate_ = node_->create_publisher<vino_people_msgs::msg::LicensePlateStamped>(
    "/openvino_toolkit/" + output_name_ + "/detected_license_plates", 16);
  pub_vehicle_attribs_ = node_->create_publisher<vino_people_msgs::msg::VehicleAttribsStamped>(
    "/openvino_toolkit/" + output_name_ + "/detected_vehicles_attribs", 16);
  pub_landmarks_ = node_->create_publisher<vino_people_msgs::msg::LandmarkStamped>(
    "/openvino_toolkit/" + output_name_ + "/detected_landmarks", 16);
  pub_face_reid_ = node_->create_publisher<vino_people_msgs::msg::ReidentificationStamped>(
    "/openvino_toolkit/" + output_name_ + "/reidentified_faces", 16);
  pub_person_attribs_ = node_->create_publisher<vino_people_msgs::msg::PersonAttributeStamped>(
    "/openvino_toolkit/" + output_name_ + "/person_attributes", 16);
  pub_person_reid_ = node_->create_publisher<vino_people_msgs::msg::ReidentificationStamped>(
    "/openvino_toolkit/" + output_name_ + "/reidentified_persons", 16);
  pub_segmented_object_ = node_->create_publisher<vino_people_msgs::msg::ObjectsInMasks>(
  ...
  pub_emotion_ =
    node_->create_publisher<vino_people_msgs::msg::EmotionsStamped>(
    "/openvino_toolkit/" + output_name_ + "/emotions", 16);
  pub_age_gender_ = node_->create_publisher<vino_people_msgs::msg::AgeGenderStamped>(
    "/openvino_toolkit/" + output_name_ + "/age_genders", 16);
  pub_headpose_ =
    node_->create_publisher<vino_people_msgs::msg::HeadPoseStamped>(
...
void Outputs::RosTopicOutput::accept(
  const std::vector<vino_core_lib::VehicleAttribsDetectionResult> & results)
{
  vehicle_attribs_topic_ = std::make_shared<vino_people_msgs::msg::VehicleAttribsStamped>();
  vino_people_msgs::msg::VehicleAttribs attribs;
  for (auto & r : results) {
    // slog::info << ">";
...
void Outputs::RosTopicOutput::accept(
  const std::vector<vino_core_lib::LicensePlateDetectionResult> & results)
{
  license_plate_topic_ = std::make_shared<vino_people_msgs::msg::LicensePlateStamped>();
  vino_people_msgs::msg::LicensePlate plate;
  for (auto & r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
void Outputs::RosTopicOutput::accept(
  const std::vector<vino_core_lib::FaceReidentificationResult> & results)
{
  face_reid_topic_ = std::make_shared<vino_people_msgs::msg::ReidentificationStamped>();
  vino_people_msgs::msg::Reidentification face;
  for (auto & r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
void Outputs::RosTopicOutput::accept(
  const std::vector<vino_core_lib::LandmarksDetectionResult> & results)
{
  landmarks_topic_ = std::make_shared<vino_people_msgs::msg::LandmarkStamped>();
  vino_people_msgs::msg::Landmark landmark;
  for (auto & r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
void Outputs::RosTopicOutput::accept(
  const std::vector<vino_core_lib::PersonAttribsDetectionResult> & results)
{
  person_attribs_topic_ = std::make_shared<vino_people_msgs::msg::PersonAttributeStamped>();
  vino_people_msgs::msg::PersonAttribute person_attrib;
  for (auto & r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
{
  person_reid_topic_ = std::make_shared<vino_people_msgs::msg::ReidentificationStamped>();
  vino_people_msgs::msg::Reidentification person;
  for (auto & r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
{
  segmented_objects_topic_ = std::make_shared<vino_people_msgs::msg::ObjectsInMasks>();
  vino_people_msgs::msg::ObjectInMask object;
  for (auto & r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
{
  emotions_topic_ = std::make_shared<vino_people_msgs::msg::EmotionsStamped>();

  vino_people_msgs::msg::Emotion emotion;
  for (auto r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
{
  age_gender_topic_ = std::make_shared<vino_people_msgs::msg::AgeGenderStamped>();

  vino_people_msgs::msg::AgeGender ag;
  for (auto r : results) {
    // slog::info << ">";
    auto loc = r.getLocation();
...
{
  headpose_topic_ = std::make_shared<vino_people_msgs::msg::HeadPoseStamped>();

  vino_people_msgs::msg::HeadPose hp;
  for (auto r : results) {
    auto loc = r.getLocation();
