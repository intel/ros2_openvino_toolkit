  ...
  std::shared_ptr<typename T::Response> response)
{
  for (auto it = pipelines_->begin(); it != pipelines_->end(); ++it) {
    vino_pipeline_srv_msgs::msg::Pipeline pipeline_msg;
    ...
    for (auto & current_pipe : connection_map) {
      vino_pipeline_srv_msgs::msg::Connection connection;
...
template class PipelineProcessingServer<vino_pipeline_srv_msgs::srv::PipelineSrv>;
}  // namespace vino_service
