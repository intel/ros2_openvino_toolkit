...
vino_core_lib::ObjectDetectionResult::ObjectDetectionResult(const cv::Rect & location)
: Result(location)
{
}
...
: show_output_thresh_(show_output_thresh),
  enable_roi_constraint_(enable_roi_constraint), vino_core_lib::BaseInference()
{
...
bool vino_core_lib::ObjectDetection::enqueue(
  const cv::Mat & frame,
...
const vino_core_lib::ObjectDetection::Result *
...
void vino_core_lib::ObjectDetection::observeOutput(
...
const std::vector<cv::Rect> vino_core_lib::ObjectDetection::getFilteredROIs(
...
std::vector<cv::Rect>
vino_core_lib::ObjectDetectionResultFilter::getFilteredLocations()
{
...
bool vino_core_lib::ObjectDetectionResultFilter::isValidLabel(
  const Result & result, const std::string & op, const std::string & target)
{
  return stringCompare(result.getLabel(), op, target);
}

bool vino_core_lib::ObjectDetectionResultFilter::isValidConfidence(
  const Result & result, const std::string & op, const std::string & target)
{
  return floatCompare(result.getConfidence(), op, stringToFloat(target));
}

bool vino_core_lib::ObjectDetectionResultFilter::isValidResult(
  const Result & result)
{
  ISVALIDRESULT(key_to_function_, result);
}

double vino_core_lib::ObjectDetection::calcIoU(
  const cv::Rect & box_1,
  const cv::Rect & box_2)
{
