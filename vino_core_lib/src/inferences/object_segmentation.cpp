...
vino_core_lib::ObjectSegmentationResult::ObjectSegmentationResult(const cv::Rect &location)
    : Result(location)
{
}
...
void vino_core_lib::ObjectSegmentation::loadNetwork(
 ...
 */
bool vino_core_lib::ObjectSegmentation::enqueue_for_one_input(
    const cv::Mat &frame,
    const cv::Rect &input_frame_loc)
{
  ...
  if (!vino_core_lib::BaseInference::enqueue<u_int8_t>(frame, input_frame_loc, 1, 0,
...
bool vino_core_lib::ObjectSegmentation::enqueue(
    const cv::Mat &frame,
    const cv::Rect &input_frame_loc)
{
  if (width_ == 0 && height_ == 0)
...
vino_core_lib::ObjectSegmentation::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void vino_core_lib::ObjectSegmentation::observeOutput(
...
const std::vector<cv::Rect> vino_core_lib::ObjectSegmentation::getFilteredROIs(
