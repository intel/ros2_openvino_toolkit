...
#include <vino_people_msgs/msg/reidentification.hpp>
#include <vino_people_msgs/msg/reidentification_stamped.hpp>
#include <vino_people_msgs/msg/landmark.hpp>
#include <vino_people_msgs/msg/landmark_stamped.hpp>
...
{
  auto node = rclcpp::Node::make_shared("openvino_face_detection");
...

  {
    ...
    EXPECT_TRUE(face_detection);
  }
}
TEST(UnitTestFaceReidentification, testLandmarkDetection)
{
  auto node = rclcpp::Node::make_shared("openvino_landmark_detection");
  auto qos = rclcpp::QoS(rclcpp::KeepLast(1)).best_effort();
  std::promise<bool> sub_called;
  std::shared_future<bool> sub_called_future(sub_called.get_future());

  auto openvino_landmark_detection_callback =
    [&sub_called](const vino_people_msgs::msg::LandmarkStamped::SharedPtr msg) -> void {
  ...
  {
    auto sub1 = node->create_subscription<vino_people_msgs::msg::LandmarkStamped>(
    ...
    EXPECT_TRUE(landmark_detection);
  }
}
TEST(UnitTestFaceReidentification, testReidentification)
{
  ...
  auto openvino_face_reidentification_callback =
    [&sub_called](const vino_people_msgs::msg::ReidentificationStamped::SharedPtr msg) -> void {
  ...
  {
    auto sub1 = node->create_subscription<vino_people_msgs::msg::ReidentificationStamped>(
...
int main(int argc, char * argv[])
{
  ...
  system("ros2 launch vino_test pipeline_face_reidentification_test.launch.py &");
