...
"""
ROS 2 Launch File: People Analytics Pipeline (IP Camera Input)

Launches the OpenVINO people detection pipeline optimized for IP camera input.
Performs face detection with age/gender, emotion, and head pose estimation.

This variant is optimized for network camera streams.
...
def generate_launch_description():
    """Generate launch description for people analytics with IP camera and RViz2."""

    # Get package share directory
    package_share_dir = get_package_share_directory('openvino_node')

    # Resolve YAML configuration paths
    resolved_yaml = launch_helpers.resolve_yaml_paths('pipeline_people_ip.yaml')
    ...
    return LaunchDescription([
        ...
        # OpenVINO people analytics node (IP camera optimized)
        Node(
