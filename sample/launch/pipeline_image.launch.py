...
"""
ROS 2 Launch File: People Analytics Pipeline on Static Images

Launches the OpenVINO people detection pipeline for processing static images.
Performs face detection with age/gender, emotion, and head pose estimation.

This pipeline reads from image files (not live camera feed).
...
def generate_launch_description():
    """Generate launch description for people analytics on images with RViz2."""

    # Get package share directory
    package_share_dir = get_package_share_directory('openvino_node')

    # Resolve YAML configuration paths
    resolved_yaml = launch_helpers.resolve_yaml_paths('pipeline_image.yaml')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description='Path to YAML configuration file for the OpenVINO image pipeline'
        ),
        ...
        # OpenVINO people analytics node for images
        Node(
            ...
            remappings=[
                ...
                ('/openvino_toolkit/people/age_genders',
                 '/ros2_openvino_toolkit/people/age_genders_Recognition'),
                ('/openvino_toolkit/people/images',
                 '/ros2_openvino_toolkit/image_rviz'),
            ],
            output='screen'
        ),
