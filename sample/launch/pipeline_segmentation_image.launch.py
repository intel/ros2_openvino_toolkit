...
"""
ROS 2 Launch File: Semantic Segmentation Pipeline on Static Images

Launches the OpenVINO semantic segmentation pipeline for processing static images.
Performs pixel-wise classification (not real-time camera stream).
...
Topics:
- Subscribes to: /camera/color/image_raw (or image file input)
...
def generate_launch_description():
    """Generate launch description for semantic segmentation on images with RViz2."""

    # Get package share directory
    package_share_dir = get_package_share_directory('openvino_node')

    # Resolve YAML configuration paths
    resolved_yaml = launch_helpers.resolve_yaml_paths('pipeline_segmentation_image.yaml')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description='Path to YAML configuration file for the segmentation pipeline'
        ),
        ...
        # OpenVINO semantic segmentation node
        Node(
