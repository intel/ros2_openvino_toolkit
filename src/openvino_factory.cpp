...
namespace openvino
{
OpenVINOFactory::OpenVINOFactory(const std::string & node_name, bool is_intra_process)
: Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(is_intra_process)),
  logger_(rclcpp::get_logger(node_name))
{
  ...
  if (iter == MODEL_MAP.end()) {
    RCLCPP_INFO(logger_, "Unsupported model type: %s", model_type.c_str());
...
{
  int cnt = 0;
  RCLCPP_INFO(logger_, "Current supported CNN model:");
  for (auto iter = MODEL_MAP.begin(); iter != MODEL_MAP.end(); iter++) {
    RCLCPP_INFO(logger_, "%d: %s", cnt, iter->first.c_str());
    cnt++;
  }
}
}  // namespace openvino
