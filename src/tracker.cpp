...
int Tracker::processNewTracker(const std::vector<float> & feature)
{
  int most_similar_id;
...
double Tracker::calcSimilarity(
  const std::vector<float> & feature_a, const std::vector<float> & feature_b)
{
  if (feature_a.size() != feature_b.size()) {
    std::cerr << "cosine similarity can't be called for vectors of different lengths: " <<
  ...
  int track_id, const std::vector<float> & feature)
{
  if (recorded_tracks_.find(track_id) != recorded_tracks_.end()) {
    ...
    std::cerr << "updating a non-existing track.";
  }
}
...
bool Tracker::saveTracksToFile(std::string filepath)
{
  ...
  if (!outfile.is_open()) {
  std::cerr << "file not exists in file path: " << filepath;
    return false;
  }
  ...
  outfile.close();
  std::cerr << "sucessfully save tracks into file: " << filepath;
  return true;
}
...
bool Tracker::loadTracksFromFile(std::string filepath)
{
  ...
  infile.close();
  std::cerr << "sucessfully load tracks from file: " << filepath;
  return true;
}
