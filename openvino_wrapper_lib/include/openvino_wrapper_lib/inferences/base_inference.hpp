 ...
 */
template <typename T>
void matU8ToBlob(const cv::Mat& orig_image, ov::Tensor& input_tensor, float scale_factor = 1.0, int batch_index = 0)
{
...
public:
  ...
  virtual const std::vector<cv::Rect> getFilteredROIs(const std::string filter_conditions) const = 0;
...
protected:
   ...
   */
  template <typename T>
  bool enqueue(const cv::Mat& frame, const cv::Rect&, float scale_factor, int batch_index,
