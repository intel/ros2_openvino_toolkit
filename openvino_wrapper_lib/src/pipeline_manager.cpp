...
PipelineManager::createObjectSegmentation(const Params::ParamManager::InferenceRawData& infer)
{
  ...
  auto segmentation_inference_ptr =
      std::make_shared<openvino_wrapper_lib::ObjectSegmentation>(infer.confidence_threshold);
...
{
  auto model = std::make_shared<Models::ObjectSegmentationMaskrcnnModel>(infer.label, infer.model, infer.batch);
  model->modelInit();
  slog::info << "Segmentation model initialized." << slog::endl;
  auto engine = engine_manager_.createEngine(infer.engine, model);
  slog::info << "Segmentation Engine initialized." << slog::endl;
  auto segmentation_inference_ptr =
      std::make_shared<openvino_wrapper_lib::ObjectSegmentationMaskrcnn>(infer.confidence_threshold);
  slog::info << "Segmentation Inference instanced." << slog::endl;
  segmentation_inference_ptr->loadNetwork(model);
  segmentation_inference_ptr->loadEngine(engine);
...
PipelineManager::createObjectSegmentationInstance(const Params::ParamManager::InferenceRawData& infer)
{
  ...
  auto segmentation_inference_ptr =
      std::make_shared<openvino_wrapper_lib::ObjectSegmentationInstance>(infer.confidence_threshold);
  slog::info << "Segmentation Inference instanced." << slog::endl;
  segmentation_inference_ptr->loadNetwork(model);
  segmentation_inference_ptr->loadEngine(engine);
...
PipelineManager::createVehicleAttribsDetection(const Params::ParamManager::InferenceRawData& infer)
{
  ...
  model->modelInit();
  auto engine = engine_manager_.createEngine(infer.engine, model);
  auto vehicle_attribs_ptr = std::make_shared<openvino_wrapper_lib::VehicleAttribsDetection>();
...
PipelineManager::createLicensePlateDetection(const Params::ParamManager::InferenceRawData& infer)
{
  ...
  model->modelInit();
  auto engine = engine_manager_.createEngine(infer.engine, model);
  auto license_plate_ptr = std::make_shared<openvino_wrapper_lib::LicensePlateDetection>();
