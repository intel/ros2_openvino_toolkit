...
Models::AgeGenderDetectionModel::AgeGenderDetectionModel(const std::string& label_loc, const std::string& model_loc,
                                                         int max_batch_size)
  : BaseModel(label_loc, model_loc, max_batch_size)
{
}
bool Models::AgeGenderDetectionModel::updateLayerProperty(std::shared_ptr<ov::Model>& model)
{
  ...
  if (inputs_info_.size() != 1) {
    slog::warn << "This model seems not Age-Gender-like, which should have only one input, but we got"
               << std::to_string(input_tensor_shape.size()) << "inputs" << slog::endl;
    return false;
  }
  ...
  if (outputs_info_.size() != 2) {
    slog::warn << "This model seems not Age-Gender-like, which should have and only have 2 outputs, but we got"
               << std::to_string(outputs_info_.size()) << "outputs" << slog::endl;
    return false;
  }
  ...
  ppp.output(age_output_info.get_any_name()).tensor().set_element_type(ov::element::f32).set_layout(tensor_layout);
  auto gender_output_info = outputs_info_[0];
  ppp.output(gender_output_info.get_any_name()).tensor().set_element_type(ov::element::f32).set_layout(tensor_layout);
