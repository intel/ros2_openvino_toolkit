...
Models::ObjectSegmentationInstanceMaskrcnnModel::ObjectSegmentationInstanceMaskrcnnModel(const std::string& label_loc,
...
bool Models::ObjectSegmentationInstanceMaskrcnnModel::updateLayerProperty(std::shared_ptr<ov::Model>& model)
{
    ...
    const bool& enable_roi_constraint)
{
  ...
  for (size_t box = 0; box < box_num; ++box) {
    ...
    if (prob > confidence_thresh) {
      ...
      std::string label =
          class_id < labels.size() ? labels[class_id] : std::string("label #") + std::to_string(class_id);
