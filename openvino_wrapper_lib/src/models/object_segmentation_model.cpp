...
Models::ObjectSegmentationModel::ObjectSegmentationModel(const std::string& label_loc, const std::string& model_loc,
                                                         int max_batch_size)
...
bool Models::ObjectSegmentationModel::updateLayerProperty(std::shared_ptr<ov::Model>& model)
{
  ...
  if (inputs_info_.size() != 1) {
    slog::warn << "This inference sample should have only one input, but we got" << std::to_string(inputs_info_.size())
               << "inputs" << slog::endl;
    return false;
  }
  ...
  input_info.tensor().set_element_type(ov::element::u8).set_layout(tensor_layout).set_spatial_dynamic_shape();
  ...
  if (outputs_info.size() != 1) {
    slog::warn << "This inference sample should have only one output, but we got" << std::to_string(outputs_info.size())
               << "outputs" << slog::endl;
    return false;
  }
  ...
  switch (outSizeVector.size()) {
    ...
    default:
      throw std::runtime_error(
          "Unexpected output blob shape. Only 4D and 3D output blobs are"
          "supported.");
  }
