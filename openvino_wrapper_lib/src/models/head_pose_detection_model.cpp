...
Models::HeadPoseDetectionModel::HeadPoseDetectionModel(const std::string& label_loc, const std::string& model_loc,
                                                       int max_batch_size)
  : BaseModel(label_loc, model_loc, max_batch_size)
{
}

bool Models::HeadPoseDetectionModel::updateLayerProperty(std::shared_ptr<ov::Model>& model)
{
  ...
  input_info.tensor().set_element_type(ov::element::u8).set_layout(input_tensor_layout);
  ...
  for (auto& output_item : output_info_map) {
    ...
    ppp.output(output_tensor_name_).tensor().set_element_type(ov::element::f32).set_layout(output_tensor_layout);
