...
Models::ObjectSegmentationMaskrcnnModel::ObjectSegmentationMaskrcnnModel(const std::string& label_loc,
...
bool Models::ObjectSegmentationMaskrcnnModel::enqueue(const std::shared_ptr<Engines::Engine>& engine,
...
bool Models::ObjectSegmentationMaskrcnnModel::matToBlob(const cv::Mat& orig_image, const cv::Rect&, float scale_factor,
                                                        int batch_index, const std::shared_ptr<Engines::Engine>& engine)
{
  ...
  if (channels == 1) {
    ...
    for (size_t c = 0; c < channels; c++) {
      for (size_t h = 0; h < height; h++) {
        for (size_t w = 0; w < width; w++) {
          input_data[batchOffset + c * width * height + h * width + w] = resized_image.at<cv::Vec3b>(h, w)[c];
        }
      }
    }
  } else {
...
bool Models::ObjectSegmentationMaskrcnnModel::updateLayerProperty(std::shared_ptr<ov::Model>& model)
{
  ...
  if (inputs_info_.size() != 2) {
    slog::warn << "This inference sample should have have two inputs, but we got" << std::to_string(inputs_info_.size())
               << "inputs" << slog::endl;
    return false;
  }
  ...
  slog::debug << "netBatchSize=" << netBatchSize << ", netInputHeight=" << netInputHeight
              << ", netInputWidth=" << netInputWidth << slog::endl;

  // check output shape
  outputs_info_ = model->outputs();
  slog::debug << "output size=" << outputs_info_.size() << slog::endl;
  if (outputs_info_.size() != 2) {
    slog::warn << "This inference sample should have have 2 outputs, but we got" << std::to_string(outputs_info_.size())
               << "outputs" << slog::endl;
    return false;
  }
