...
Models::EmotionDetectionModel::EmotionDetectionModel(const std::string& label_loc, const std::string& model_loc,
                                                     int max_batch_size)
  : BaseModel(label_loc, model_loc, max_batch_size)
{
}

bool Models::EmotionDetectionModel::updateLayerProperty(std::shared_ptr<ov::Model>& model)
{
  ...
  if (outputs_info_.size() != 1) {
    slog::warn << "This model should have and only have 1 output, but we got " << std::to_string(outputs_info_.size())
               << "outputs" << slog::endl;
    return false;
  }
