...
Models::ObjectSegmentationInstanceModel::ObjectSegmentationInstanceModel(const std::string& label_loc,
...
bool Models::ObjectSegmentationInstanceModel::enqueue(const std::shared_ptr<Engines::Engine>& engine,
...
bool Models::ObjectSegmentationInstanceModel::updateLayerProperty(std::shared_ptr<ov::Model>& model)
{
    ...
    const bool& enable_roi_constraint)
{
  ...
  slog::debug << "mask_output_shape: " << mask_output_shape << ",MASK_HEIGHT:" << MASK_HEIGHT
              << ", MASK_WIDTH:" << MASK_WIDTH << slog::endl;
