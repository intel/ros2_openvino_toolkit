 ...
 */
bool openvino_wrapper_lib::ObjectSegmentationMaskrcnn::enqueue_for_one_input(const cv::Mat& frame,
...
bool openvino_wrapper_lib::ObjectSegmentationMaskrcnn::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (width_ == 0 && height_ == 0) {
    width_ = frame.cols;
    height_ = frame.rows;
...
const openvino_wrapper_lib::Result* openvino_wrapper_lib::ObjectSegmentationMaskrcnn::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void openvino_wrapper_lib::ObjectSegmentationMaskrcnn::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr) {
...
openvino_wrapper_lib::ObjectSegmentationMaskrcnn::getFilteredROIs(const std::string filter_conditions) const
{
