...
openvino_wrapper_lib::FaceReidentificationResult::FaceReidentificationResult(const cv::Rect& location)
  : Result(location)
{
}
...
bool openvino_wrapper_lib::FaceReidentification::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (getEnqueuedNum() == 0) {
    results_.clear();
  }
...
const openvino_wrapper_lib::Result* openvino_wrapper_lib::FaceReidentification::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void openvino_wrapper_lib::FaceReidentification::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr) {
