...
bool openvino_wrapper_lib::ObjectSegmentationInstance::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
...
bool openvino_wrapper_lib::ObjectSegmentationInstance::fetchResults()
{
  ...
  return (valid_model_ != nullptr) && valid_model_->fetchResults(getEngine(), results_, show_output_thresh_);
}
...
const openvino_wrapper_lib::Result* openvino_wrapper_lib::ObjectSegmentationInstance::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void openvino_wrapper_lib::ObjectSegmentationInstance::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr) {
...
openvino_wrapper_lib::ObjectSegmentationInstance::getFilteredROIs(const std::string filter_conditions) const
{
