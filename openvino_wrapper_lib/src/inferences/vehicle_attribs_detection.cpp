...
openvino_wrapper_lib::VehicleAttribsDetectionResult::VehicleAttribsDetectionResult(const cv::Rect& location)
  : Result(location)
{
}

// VehicleAttribsDetection
openvino_wrapper_lib::VehicleAttribsDetection::VehicleAttribsDetection() : openvino_wrapper_lib::BaseInference()
{
}
...
bool openvino_wrapper_lib::VehicleAttribsDetection::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (getEnqueuedNum() == 0) {
    results_.clear();
  }
...
const openvino_wrapper_lib::Result* openvino_wrapper_lib::VehicleAttribsDetection::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void openvino_wrapper_lib::VehicleAttribsDetection::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr) {
...
openvino_wrapper_lib::VehicleAttribsDetection::getFilteredROIs(const std::string filter_conditions) const
{
