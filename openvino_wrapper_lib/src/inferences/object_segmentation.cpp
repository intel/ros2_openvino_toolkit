...
openvino_wrapper_lib::ObjectSegmentationResult::ObjectSegmentationResult(const cv::Rect& location) : Result(location)
{
}
 ...
 */
bool openvino_wrapper_lib::ObjectSegmentation::enqueue_for_one_input(const cv::Mat& frame,
...
bool openvino_wrapper_lib::ObjectSegmentation::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (width_ == 0 && height_ == 0) {
    width_ = frame.cols;
    height_ = frame.rows;
...
const openvino_wrapper_lib::Result* openvino_wrapper_lib::ObjectSegmentation::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void openvino_wrapper_lib::ObjectSegmentation::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr) {
...
openvino_wrapper_lib::ObjectSegmentation::getFilteredROIs(const std::string filter_conditions) const
{
  if (!filter_conditions.empty()) {
    slog::err << "Object segmentation does not support filtering now! " << "Filter conditions: " << filter_conditions
              << slog::endl;
  }
