...
openvino_wrapper_lib::LandmarksDetectionResult::LandmarksDetectionResult(const cv::Rect& location) : Result(location)
{
}

// LandmarksDetection
openvino_wrapper_lib::LandmarksDetection::LandmarksDetection() : openvino_wrapper_lib::BaseInference()
{
}
...
bool openvino_wrapper_lib::LandmarksDetection::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (getEnqueuedNum() == 0) {
    results_.clear();
  }
...
const openvino_wrapper_lib::Result* openvino_wrapper_lib::LandmarksDetection::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void openvino_wrapper_lib::LandmarksDetection::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr) {
...
openvino_wrapper_lib::LandmarksDetection::getFilteredROIs(const std::string filter_conditions) const
{
  if (!filter_conditions.empty()) {
    slog::err << "Landmarks detection does not support filtering now! " << "Filter conditions: " << filter_conditions
              << slog::endl;
  }
